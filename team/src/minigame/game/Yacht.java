...
public class Yacht {
	...
	private void printTest(YachtVariable yv) {
		...
		// 사용자 입력 받기
		gameResult += "\n다시 굴릴 주사위 번호를 입력하세요 (예: 1 2 4, 종료는 0, 남은 기회 : " + (2 - yv.reRollCount) + "번) |종료: EXIT| : ";
	}
	...
	public void run(Message message) {
		...
		if (currentTurn.equals(player1)) {
			if (p1.dices[0] == 0) {
				...
				if (p1.reRollCount != 2) {
					gameResult += "\n다시 굴릴 주사위 번호를 입력하세요 (예: 1 2 4, 종료는 0, 남은 기회 : " + (2 - p1.reRollCount) + "번) |종료: EXIT| : ";
				}
			}

			if (p1.reRollCount == 2) {
				gameResult += "\n1~12 중 어디에 넣을 건지를 선택하세요 |종료: EXIT| : ";
				p1.reRollCount++;
		...
		if (currentTurn.equals(player2)) {
			if (p2.dices[0] == 0) {
				...
				if (p2.reRollCount != 2) {
					gameResult += "\n다시 굴릴 주사위 번호를 입력하세요 (예: 1 2 4, 종료는 0, 남은 기회 : " + (2 - p2.reRollCount) + "번) |종료: EXIT| : ";
				}
			}
			if (p2.reRollCount == 2) {
				gameResult += "\n1~12 중 어디에 넣을 건지를 선택하세요 |종료: EXIT| : ";
