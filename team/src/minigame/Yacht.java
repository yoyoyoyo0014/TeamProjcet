...
public class Yacht {
	...
	private static int diceReroll(int[] dices, int r) {
		...
		while (true) {
			// 사용자 입력 받기
			gameResult += "다시 굴릴 주사위 번호를 입력하세요 (예: 1 2 4, 종료는 0, 남은 기회 : " + r + "번): ";
	...
	private void print() {
		...
		if (p1.oneBl == true && p2.oneBl == true) {
			gameResult += "|1. Aces\t | " + blue + oneCal(p1.dices, p1.one, p1.oneBl) + exit + "\t     | " + blue + oneCal(p2.dices, p2.one, p2.oneBl)
					+ exit + "\t\t   |\n";
		} else if (p1.oneBl == true) {
			gameResult += "|1. Aces\t | " + blue + oneCal(p1.dices, p1.one, p1.oneBl) + exit + "\t     | " + oneCal(p2.dices, p2.one, p2.oneBl) + "\t   |\n";
		} else if (p2.oneBl == true) {
			gameResult += "|1. Aces\t | " + oneCal(p1.dices, p1.one, p1.oneBl) + "\t  | " + blue + oneCal(p2.dices, p2.one, p2.oneBl) + exit + "\t\t   |\n";
		} else {
			gameResult += "|1. Aces\t | " + oneCal(p1.dices, p1.one, p1.oneBl) + "\t  | " + oneCal(p2.dices, p2.one, p2.oneBl) + "\t   |\n";
		}

		if (p1.twoBl == true && p2.twoBl == true && p1.two >= 10 && p2.two >= 10) {
			gameResult += "|2. Deuces\t | " + blue + twoCal(p1.dices, p1.two, p1.twoBl) + exit + "\t    | " + blue + twoCal(p2.dices, p2.two, p2.twoBl)
					+ exit + "\t    |\n";
			...
			gameResult += "|2. Deuces\t | " + blue + twoCal(p1.dices, p1.two, p1.twoBl) + exit + "\t    | " + twoCal(p2.dices, p2.two, p2.twoBl) + "\t   |\n";
			...
			gameResult += "|2. Deuces\t | " + twoCal(p1.dices, p1.two, p1.twoBl) + "\t  | " + blue + twoCal(p2.dices, p2.two, p2.twoBl) + exit + "\t\t   |\n";
		} else {
			gameResult += "|2. Deuces\t | " + twoCal(p1.dices, p1.two, p1.twoBl) + "\t  | " + twoCal(p2.dices, p2.two, p2.twoBl) + "\t   |\n";
		}
		...
		if (p1.threeBl == true && p2.threeBl == true && p1.three >= 10 && p2.three >= 10) {
			...
			gameResult += "|3. Threes\t | " + threeCal(p1.dices, p1.three, p1.threeBl) + "\t  | " + blue + threeCal(p2.dices, p2.three, p2.threeBl)
					+ exit + "\t\t   |\n";
		} else {
			gameResult += "|3. Threes\t | " + threeCal(p1.dices, p1.three, p1.threeBl) + "\t  | " + threeCal(p2.dices, p2.three, p2.threeBl) + "\t   |\n";
		}
		...
		if (p1.fourBl == true && p2.fourBl == true && p1.four >= 10 && p2.four >= 10) {
			...
			gameResult += "|4. Fours\t | " + fourCal(p1.dices, p1.four, p1.fourBl) + "\t  | " + fourCal(p2.dices, p2.four, p2.fourBl) + "\t   |\n";
		}
		...
		if (p1.fiveBl == true && p2.fiveBl == true && p1.five >= 10 && p2.five >= 10) {
			...
			gameResult += "|5. Fives\t | " + fiveCal(p1.dices, p1.five, p1.fiveBl) + "\t  | " + fiveCal(p2.dices, p2.five, p2.fiveBl) + "\t   |\n";
		}

		if (p1.sixBl == true && p2.sixBl == true && p1.six >= 10 && p2.six >= 10) {
			gameResult += "|6. Sixes\t | " + blue + sixCal(p1.dices, p1.six, p1.sixBl) + exit + "\t    | " + blue + sixCal(p2.dices, p2.six, p2.sixBl)
					+ exit + "\t    |\n";
			...
			gameResult += "|6. Sixes\t | " + blue + sixCal(p1.dices, p1.six, p1.sixBl) + exit + "\t    | " + sixCal(p2.dices, p2.six, p2.sixBl) + "\t   |\n";
		} else if (p1.sixBl == true) {
			gameResult += "|6. Sixes\t | " + blue + sixCal(p1.dices, p1.six, p1.sixBl) + exit + "\t     | " + sixCal(p2.dices, p2.six, p2.sixBl) + "\t   |\n";
		} else if (p2.sixBl == true) {
			gameResult += "|6. Sixes\t | " + sixCal(p1.dices, p1.six, p1.sixBl) + "\t  | " + blue + sixCal(p2.dices, p2.six, p2.sixBl) + exit + "\t\t   |\n";
		} else {
			gameResult += "|6. Sixes\t | " + sixCal(p1.dices, p1.six, p1.sixBl) + "\t  | " + sixCal(p2.dices, p2.six, p2.sixBl) + "\t   |\n";
		}
		gameResult += "|==================================|\n";

		// 보너스
		if (p1.bonusBl == true && p2.bonusBl == true) {
			gameResult += "|Subtotal\t | " + blue + p1.oneToSix + "/63" + exit + "\t | " + blue + p2.oneToSix + "/63" + exit + "\t |\n";
			gameResult += "|----------------------------------|\n";
			gameResult += "|+35 Bonus\t | " + blue + "+" + bonusCal(p1.bonusBl) + exit + "\t\t  | " + blue + "+" + bonusCal(p2.bonusBl) + exit + "\t   |\n";
		} else if (p1.bonusBl == true && p2.oneToSix >= 10) {
			gameResult += "|Subtotal\t | " + blue + p1.oneToSix + "/63" + exit + "\t | " + p2.oneToSix + "/63" + "  |\n";
			gameResult += "|----------------------------------|\n";
			gameResult += "|+35 Bonus\t | " + blue + "+" + bonusCal(p1.bonusBl) + exit + "\t\t  | " + "+" + bonusCal(p2.bonusBl) + "\t   |\n";
		} else if (p1.bonusBl == true && p2.oneToSix < 10) {
			gameResult += "|Subtotal\t | " + blue + p1.oneToSix + "/63" + exit + "\t | " + p2.oneToSix + "/63" + "   |\n";
			gameResult += "|----------------------------------|\n";
			gameResult += "|+35 Bonus\t | " + blue + "+" + bonusCal(p1.bonusBl) + exit + "\t\t  | " + "+" + bonusCal(p2.bonusBl) + "\t   |\n";
			...
			gameResult += "|----------------------------------|\n";
			gameResult += "|+35 Bonus\t | " + "+" + bonusCal(p1.bonusBl) + "\t  | " + blue + "+" + bonusCal(p2.bonusBl) + exit + "\t   |\n";
		} else if (p2.bonusBl == true && p1.oneToSix < 10) {
			gameResult += "|Subtotal\t | " + p1.oneToSix + "/63" + "\t  | " + blue + p2.oneToSix + "/63" + exit + "  |\n";
			gameResult += "|----------------------------------|\n";
			gameResult += "|+35 Bonus\t | " + "+" + bonusCal(p1.bonusBl) + "\t  | " + blue + "+" + bonusCal(p2.bonusBl) + exit + "\t   |\n";
		...
		if (p1.chBl == true && p2.chBl == true && p1.ch >= 10 && p2.ch >= 10) {
			...
			gameResult += "|7. Choice\t | " + blue + chCal(p1.dices, p1.ch, p1.chBl) + exit + "\t    | " + blue + chCal(p2.dices, p2.ch, p2.chBl)
					+ exit + "\t     |\n";
		} else if (p1.chBl == true && p2.chBl == true && p1.ch < 10 && p2.ch >= 10) {
			gameResult += "|7. Choice\t | " + blue + chCal(p1.dices, p1.ch, p1.chBl) + exit + "\t     | " + blue + chCal(p2.dices, p2.ch, p2.chBl)
					+ exit + "\t    |\n";
			...
			gameResult += "|7. Choice\t | " + blue + chCal(p1.dices, p1.ch, p1.chBl) + exit + "\t    | " + chCal(p2.dices, p2.ch, p2.chBl) + "\t   |\n";
		} else if (p1.chBl == true) {
			gameResult += "|7. Choice\t | " + blue + chCal(p1.dices, p1.ch, p1.chBl) + exit + "\t     | " + chCal(p2.dices, p2.ch, p2.chBl) + "\t   |\n";
		} else if (p2.chBl == true) {
			gameResult += "|7. Choice\t | " + chCal(p1.dices, p1.ch, p1.chBl) + "\t  | " + blue + chCal(p2.dices, p2.ch, p2.chBl) + exit + "\t\t   |\n";
		} else {
		...
		if (p1.fkBl == true && p2.fkBl == true && p1.fk >= 10 && p2.fk >= 10) {
			gameResult += "|8. 4 of a Kind  | " + blue + fkCal(p1.dices, p1.fk, p1.fkBl) + exit + "\t    | " + blue + fkCal(p2.dices, p2.fk, p2.fkBl)
					+ exit + "\t    |\n";
			...
			gameResult += "|8. 4 of a Kind  | " + blue + fkCal(p1.dices, p1.fk, p1.fkBl) + exit + "\t    | " + fkCal(p2.dices, p2.fk, p2.fkBl) + "\t   |\n";
		} else if (p1.fkBl == true) {
			gameResult += "|8. 4 of a Kind  | " + blue + fkCal(p1.dices, p1.fk, p1.fkBl) + exit + "\t     | " + fkCal(p2.dices, p2.fk, p2.fkBl) + "\t   |\n";
		} else if (p2.fkBl == true) {
			gameResult += "|8. 4 of a Kind  | " + fkCal(p1.dices, p1.fk, p1.fkBl) + "\t  | " + blue + fkCal(p2.dices, p2.fk, p2.fkBl) + exit + "\t\t   |\n";
		} else {
			gameResult += "|8. 4 of a Kind  | " + fkCal(p1.dices, p1.fk, p1.fkBl) + "\t  | " + fkCal(p2.dices, p2.fk, p2.fkBl) + "\t   |\n";
		}

		if (p1.fhBl == true && p2.fhBl == true && p1.fh >= 10 && p2.fh >= 10) {
			gameResult += "|9. Full House   | " + blue + fhCal(p1.dices, p1.fh, p1.fhBl) + exit + "\t    | " + blue + fhCal(p2.dices, p2.fh, p2.fhBl)
					+ exit + "\t    |\n";
			...
			gameResult += "|9. Full House   | " + blue + fhCal(p1.dices, p1.fh, p1.fhBl) + exit + "\t    | " + fhCal(p2.dices, p2.fh, p2.fhBl) + "\t   |\n";
		} else if (p1.fhBl == true) {
			gameResult += "|9. Full House   | " + blue + fhCal(p1.dices, p1.fh, p1.fhBl) + exit + "\t     | " + fhCal(p2.dices, p2.fh, p2.fhBl) + "\t   |\n";
		} else if (p2.fhBl == true) {
			gameResult += "|9. Full House   | " + fhCal(p1.dices, p1.fh, p1.fhBl) + "\t  | " + blue + fhCal(p2.dices, p2.fh, p2.fhBl) + exit + "\t\t   |\n";
		} else {
			gameResult += "|9. Full House   | " + fhCal(p1.dices, p1.fh, p1.fhBl) + "\t  | " + fhCal(p2.dices, p2.fh, p2.fhBl) + "\t   |\n";
		}

		if (p1.ssBl == true && p2.ssBl == true && p1.ss >= 10 && p2.ss >= 10) {
			gameResult += "|10. S. Straight | " + blue + ssCal(p1.dices, p1.ss, p1.ssBl) + exit + "\t    | " + blue + ssCal(p2.dices, p2.ss, p2.ssBl)
					+ exit + "\t    |\n";
			...
			gameResult += "|10. S. Straight | " + blue + ssCal(p1.dices, p1.ss, p1.ssBl) + exit + "\t    | " + ssCal(p2.dices, p2.ss, p2.ssBl) + "\t   |\n";
		} else if (p1.ssBl == true) {
			gameResult += "|10. S. Straight | " + blue + ssCal(p1.dices, p1.ss, p1.ssBl) + exit + "\t     | " + ssCal(p2.dices, p2.ss, p2.ssBl) + "\t   |\n";
		} else if (p2.ssBl == true) {
			gameResult += "|10. S. Straight | " + ssCal(p1.dices, p1.ss, p1.ssBl) + "\t  | " + blue + ssCal(p2.dices, p2.ss, p2.ssBl) + exit + "\t\t   |\n";
		} else {
			gameResult += "|10. S. Straight | " + ssCal(p1.dices, p1.ss, p1.ssBl) + "\t  | " + ssCal(p2.dices, p2.ss, p2.ssBl) + "\t   |\n";
		}

		if (p1.lsBl == true && p2.lsBl == true && p1.ls >= 10 && p2.ls >= 10) {
			gameResult += "|11. L. Straight | " + blue + lsCal(p1.dices, p1.ls, p1.lsBl) + exit + "\t    | " + blue + lsCal(p2.dices, p2.ls, p2.lsBl)
					+ exit + "\t    |\n";
			...
			gameResult += "|11. L. Straight | " + blue + lsCal(p1.dices, p1.ls, p1.lsBl) + exit + "\t    | " + lsCal(p2.dices, p2.ls, p2.lsBl) + "\t   |\n";
		} else if (p1.lsBl == true) {
			gameResult += "|11. L. Straight | " + blue + lsCal(p1.dices, p1.ls, p1.lsBl) + exit + "\t     | " + lsCal(p2.dices, p2.ls, p2.lsBl) + "\t   |\n";
		} else if (p2.lsBl == true) {
			gameResult += "|11. L. Straight | " + lsCal(p1.dices, p1.ls, p1.lsBl) + "\t  | " + blue + lsCal(p2.dices, p2.ls, p2.lsBl) + exit + "\t\t   |\n";
		} else {
			gameResult += "|11. L. Straight | " + lsCal(p1.dices, p1.ls, p1.lsBl) + "\t  | " + lsCal(p2.dices, p2.ls, p2.lsBl) + "\t   |\n";
		}
		...
		if (p1.yaBl == true && p2.yaBl == true && p1.ya >= 10 && p2.ya >= 10) {
			...
			gameResult += "|12. Yacht\t | " + blue + yaCal(p1.dices, p1.ya, p1.yaBl) + exit + "\t    | " + blue + yaCal(p2.dices, p2.ya, p2.yaBl)
					+ exit + "\t     |\n";
		} else if (p1.yaBl == true && p2.yaBl == true && p1.ya < 10 && p2.ya >= 10) {
			gameResult += "|12. Yacht\t | " + blue + yaCal(p1.dices, p1.ya, p1.yaBl) + exit + "\t     | " + blue + yaCal(p2.dices, p2.ya, p2.yaBl)
					+ exit + "\t    |\n";
			...
			gameResult += "|12. Yacht\t | " + blue + yaCal(p1.dices, p1.ya, p1.yaBl) + exit + "\t    | " + yaCal(p2.dices, p2.ya, p2.yaBl) + "\t   |\n";
		} else if (p1.yaBl == true) {
			gameResult += "|12. Yacht\t | " + blue + yaCal(p1.dices, p1.ya, p1.yaBl) + exit + "\t     | " + yaCal(p2.dices, p2.ya, p2.yaBl) + "\t   |\n";
		} else if (p2.yaBl == true) {
			gameResult += "|12. Yacht\t | " + yaCal(p1.dices, p1.ya, p1.yaBl) + "\t  | " + blue + yaCal(p2.dices, p2.ya, p2.yaBl) + exit + "\t\t   |\n";
		} else {
